...
import com.example.abcjobsnav.repositories.AsignaAbcRepository
...
class AsignaAbcViewModel(application: Application) :  AndroidViewModel(application) {
    private val asignaAbcRepository = AsignaAbcRepository(application)

    private val _puestosSinAsig = MutableLiveData<ListaPuesto>()
    val puestosSinAsig: LiveData<ListaPuesto>
        get() = _puestosSinAsig
    ...
    public fun refreshDataFromNetwork(userId: Int, token: String, max: Int, num_pag: Int,
        ...
        try {
            viewModelScope.launch(Dispatchers.Default){
                withContext(Dispatchers.IO){
                    try{
                        var data = asignaAbcRepository.refreshData(JSONObject(postParams), userId, token, cache)
                        _puestosSinAsig.postValue(data)
    ...
    class Factory(val app: Application) : ViewModelProvider.Factory {
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            if (modelClass.isAssignableFrom(AsignaAbcViewModel::class.java)) {
                @Suppress("UNCHECKED_CAST")
                return AsignaAbcViewModel(app) as T
            }
